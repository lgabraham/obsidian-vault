...
For example, type `@todo` in the search box to find all notes with tasks, or `@files` to find all notes with attachments. Use `@images` to find ~just~ those kinds of attachments. To search specific phrases, wrap the phrase in quotes like  `"bear is cool"`. 
